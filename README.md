...
## How it works
1. On a product, open the **Documents** tab. Below the linked files, the **Vendor email images** section has a **Send images to vendors** switch (or select the product's category in setup). Untick any files that should not go out. New uploads are included by default (configurable).
...
5. Public links can be revoked per file in the same section of the Documents tab.

Other product files remain private.

**PDFs:** only `jpg,jpeg,png,gif,webp` are sent by default. Add `pdf` to the extensions in setup to send PDF drawings or spec sheets. The Vendor email images section warns about files that are skipped because of their extension.
...
| | Include new files (files never ticked or unticked on the Documents tab) | on |
